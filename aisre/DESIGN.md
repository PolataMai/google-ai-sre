...
### 14. 网关不认识推理侧(第 9–10 周)

gateway.py 不 import 任何推理侧模块(connectors/enrichment/hypotheses/
planner)——它只认三样东西:签名有效的主体令牌、通过契约校验的 ActionPlan、
绑定 plan_hash 的审批。这是"推理与执行解耦"的代码级落实:模型怎么进化,
写生产的路径始终是这条 12 环确定性检查链,任一环失败即拒绝且写审计。

### 15. fail closed 是逐环实现的,不是口号

身份验签失败、场景未注册、适配器 dry-run 抛异常、策略引擎无允许规则命中、
执行适配器崩溃——每一环的异常都被捕获并转为该环的拒绝(_Denied),
不冒泡、不放行。红色按钮在链路第一环,kill 后一切自动化立即拒绝,
kill/resume 本身也进审计。限流即 Agent 级熔断(每服务每小时上限),
并发锁保证同服务同时只有一个在途动作(LRO,mark_completed 释放)。

### 16. 人机分权在类型系统里

提交者令牌必须是 agent 主体(人工运维走原有链路,不经网关——文章要求
